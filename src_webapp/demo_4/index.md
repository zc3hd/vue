...
【!】一个DOM的不同的状态就是不通的属性值，样式名就是字符串，代表不同的状态。  
```
...
ev_send_to_f:定义了个事件名称


3.定义通道后，如何使用？
在【s】组件上绑定这个向外的通道，后面绑定【f】组件用于接收数据的方法get_from_s
...
5.子组件上的数据变化，不能直接给不父级的数据，需要通过定义的 发射事件，进行emit才可以改变。
```
...
#### 【f-->s】
* 1、初始化数据，s_cp属性绑定，
* 2、s_cp props:{}直接接受数据的类型，
* 3、在父组件中使用子组件，给指定的子组件中进行属性绑定  :attr = 'f_data' 传递数据
#### 【f<--s】
* 1、初始化组件
* 2、在ready() 或交互中，定义一个 发射事件，
* 3、在s_cp组件上绑定发射事件 @s_emit_ev 和 f_cp的方法，f_cp方法接收(参数的是默认的，不要写成f_cp()  )


#### methods方法写法：
* 有参数就是(参数)，没有参数就是直接函数名就行。
...
* 【父cp】 给 【子cp】传数据解决了。尝试进行优化这个问题的demo;
* 见demo_1_1
